...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in self.allPositions:
            observationProb = self.getObservationProb(noisyDistance, pacmanPosition, position, jailPosition)
            allPossible[position] = observationProb * beliefs[position]
    ...
    def elapseTime(self, gameState):
        ...
        for position in positions:
            ...
            for newPosition, probability in newPosDist.items():
                allPossible[newPosition] += beliefs[position] * probability
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            observationProb = self.getObservationProb(noisyDistance, pacmanPosition, particle, jailPosition)
            beliefs[particle] = observationProb + beliefs[particle] 
    ...
    def elapseTime(self, gameState):
        ...
        for position in self.legalPositions:
            ...
            for newPosition in newPosDist:
                updatedBeliefs[newPosition] += newPosDist[newPosition] * beliefs[position]
...
class JointParticleFilter(ParticleFilter):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] != None:
                    observationProb = self.getObservationProb(noisyDistances[i], pacmanPosition, particle, jailPosition)
                    beliefs[particle] = observationProb + beliefs[particle] 

            beliefs[particle] = beliefs[particle] + probability
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            # for i in range(self.numGhosts):
            #     newDeposit = self.getPositionDistribution(gameState, newParticle, i, self.ghostAgent[i])
            #     newParticle[i] = util.sample(newDeposit)
            raiseNotDefined()
